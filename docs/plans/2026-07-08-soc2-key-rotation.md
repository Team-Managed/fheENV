...
- [ ] Create `.github/workflows/rotate-scheduled.yml` with:
  ...
  - Secrets used: `FHEENV_PRIVATE_KEY` (Rotator key, not owner key), `FHEENV_PINATA_JWT`, `FHEENV_NOTIFY_WEBHOOK` (optional)
...
## Open Finding — KMS Migration (F-01)

Per spec §4.5: the Rotator credential custody via GitHub Actions encrypted secrets is acceptable to ship Phase 2 with, **provided** KMS migration is tracked as a dated remediation item rather than silently deferred.

- [ ] Open a tracked issue: "F-01: Migrate Rotator key to KMS-backed signing (AWS KMS / GCP Cloud KMS)" — target: before SOC 2 Type II surveillance audit. The Rotator key holds live decrypt access to every environment in scope; Actions-secret custody is a materially weaker choice for a credential with rewrite power.
- [ ] Document F-01 explicitly in the SOC 2 policy narrative as a disclosed remediation item with a target date — an undisclosed open finding discovered by the auditor is worse than a disclosed one with a plan.
