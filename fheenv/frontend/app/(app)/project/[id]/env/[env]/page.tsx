  ...
  return (
    <>
      ...
      >
        ...
        <Link href={`/project/${id}`} className="hover:text-slate-200 transition-colors">
          {projectName}
        </Link>
        <ChevronRight className="size-3.5" />
      ...
      {/* Page title */}
      <div
        className="flex items-center justify-between mb-8 pb-7"
        style={{ borderBottom: "1px solid var(--surface-border)" }}
      >
        <div className="flex items-center gap-3">
          <span
            className="size-3 rounded-full shrink-0"
            style={{ background: dot, boxShadow: `0 0 10px ${dot}88` }}
          />
          <div>
      ...
      {rotateLog && (
        <div
          className="mb-6 rounded-xl px-4 py-3 text-xs font-mono whitespace-pre-wrap"
          style={{
            background: "rgba(234,179,8,0.06)",
