...
export default function ProjectPage({ params }: Props) {
    ...
    return (
        <>
            ...
            {project && project[3] && (
                <div className="mb-10 pb-8" style={{ borderBottom: "1px solid var(--surface-border)" }}>
                    ...
                    <p className="text-xs font-mono mt-2" style={{ color: "var(--text-muted)" }}>
                        Owner:{" "}
                        <span className="text-slate-300">{project[1].slice(0, 10)}…{project[1].slice(-8)}</span>
                    </p>
                </div>
            )}
                    ...
                    {ENVIRONMENTS.map(({ name, label, dot, badge }) => (
                        ...
                        >
                            <div className="flex items-center gap-3.5">
                                <span className="size-2.5 rounded-full shrink-0" style={{ background: dot, boxShadow: `0 0 8px ${dot}88` }} />
                                <div>
                                    <p className="text-sm font-semibold text-slate-200">{label}</p>
                                    <p className="text-xs font-mono mt-0.5" style={{ color: "var(--text-muted)" }}>{name}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className={`text-xs px-2 py-0.5 rounded-full border font-mono ${badge}`}>{name}</span>
                                <ChevronRight className="size-4 transition-colors" style={{ color: "var(--text-muted)" }} />
                            </div>
                        </Link>
                    ))}
                </div>
            ...
            {/* CLI hint */}
            <div className="rounded-xl px-5 py-4 mb-10 flex items-start gap-3" style={{ background: "var(--surface)", border: "1px solid var(--surface-border)" }}>
