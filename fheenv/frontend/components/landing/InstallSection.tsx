...
const INSTALL_COMMANDS: Record<Platform, { label: string; command: string; icon: React.ReactNode }> = {
    mac: {
        label: "macOS / Linux",
        command: 'curl -fsSL https://raw.githubusercontent.com/Team-Managed/fheENV/main/install.sh | bash',
            ...
            <div className="max-w-3xl mx-auto px-6">
                ...
                >
                        ...
                        <div className="flex gap-1 bg-white/[0.04] rounded-lg p-1 border border-white/[0.06]">
                            {(["mac", "windows"] as Platform[]).map((p) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${(platform === p || (platform === "linux" && p === "mac"))
                    ...
                    <p className="mt-4 text-xs text-slate-500 flex items-center gap-2">
                        ...
                        Installs to <code className="text-slate-400">~/.fheenv/bin</code> · configures PATH automatically · no sudo required
                    </p>
                </motion.div>
            </div>
        </section>
