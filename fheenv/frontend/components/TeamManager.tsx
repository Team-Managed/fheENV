...
export function TeamManager({ projectId, envName }: Props) {
  ...
  async function handleGrant() {
    ...
    if (!target || !target.startsWith("0x")) {
      setGrantError("Enter a valid 0x address");
      return;
    }
  ...
  async function handleBatchGrant() {
    ...
    if (lines.length === 0) {
      setBatchError("No valid addresses found");
      return;
    }
    if (lines.length > 100) {
      setBatchError("Maximum 100 addresses per batch");
      return;
    }
  ...
  async function handleRevoke() {
    ...
    if (!target || !target.startsWith("0x")) {
      setRevokeError("Enter a valid 0x address");
      return;
    }
    ...
    >
      <div className="flex items-center gap-2.5 mb-6">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            Manage who can decrypt <span className="font-mono">{envName}</span>
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          >
            Batch grant{" "}
            <span className="normal-case font-normal">(one address per line · max 100)</span>
          </label>
          <textarea
            value={batchRaw}
            onChange={(e) => {
            ...
            >
              ...
              <div style={{ color: "#fbbf24" }}>
                ...
                <p className="font-normal" style={{ color: "rgba(251,191,36,0.7)" }}>
                  The removed member may still hold a cached AES key. Run{" "}
                  <code className="font-mono">fheenv rotate --env {envName}</code> to issue new
