...
export function SecretsTable({ projectId, envName }: Props) {
  ...
  async function handleSave() {
    if (!walletClient || !publicClient || !address) return;
    setSaving(true);
    setSaveMsg("Generating new AES key…");
    try {
    ...
    <div style={cardStyle}>
      ...
      {!loading && !secrets && !error && blobCid === "" && (
        <p className="text-xs text-center py-4" style={{ color: "var(--text-muted)" }}>
          No secrets pushed yet for <span className="font-mono">{envName}</span>.
        </p>
      )}
            ...
            >
              <table className="w-full text-xs">
                <thead>
                  ...
                  >
                    <th
                      className="text-left px-4 py-2.5 font-semibold uppercase tracking-wider"
